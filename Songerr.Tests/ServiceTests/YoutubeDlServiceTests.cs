...
public class YoutubeDlServiceTests
{
        ...
        SongModel songModel, Video result)
    {
        ...
        var mockVideo = new Video(result.Id, result.Title, mockAuthor, result.UploadDate, result.Description,
            result.Duration, result.Thumbnails, result.Keywords, result.Engagement);
        _youtubeDlClientMock
            .Setup(x => x.GetSongMetadata(It.IsAny<SongModel>()))
